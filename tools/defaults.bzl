...
def _compute_module_name():
    ...
    # Skip module name generation for Bazel packages which are not built as NPM package
    # or not scoped under `@angular/`.
    if current_pkg.startswith("src/universal-app") or \
       current_pkg.startswith("src/e2e-app") or \
       current_pkg.startswith("src/dev-app"):
        return None
